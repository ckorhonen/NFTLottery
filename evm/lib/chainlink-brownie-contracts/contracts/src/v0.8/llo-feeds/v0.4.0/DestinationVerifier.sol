    ...
    TypeAndVersionInterface
{
    ...
    function verify(bytes calldata signedReport, bytes calldata parameterPayload, address sender)
        external
        payable
        override
        onlyProxy
        ...
        returns (bytes memory)
    {
        ...
        if (fm != address(0)) {
            ...
            try IDestinationVerifierFeeManager(fm)
            .processFee{value: msg.value}(donConfigId, signedReport, parameterPayload, sender) {
            //do nothing
            }
            catch {
    ...
    /// @inheritdoc IDestinationVerifierProxyVerifier
    function verifyBulk(bytes[] calldata signedReports, bytes calldata parameterPayload, address sender)
        external
        payable
        override
        onlyProxy
        ...
        returns (bytes[] memory)
    {
        ...
        if (fm != address(0)) {
            //process the fee and catch the error
            try IDestinationVerifierFeeManager(fm)
            .processFeeBulk{value: msg.value}(donConfigs, signedReports, parameterPayload, sender) {
            //do nothing
            }
            catch {
                // we purposefully obfuscate the error here to prevent information leaking leading to free verifications
