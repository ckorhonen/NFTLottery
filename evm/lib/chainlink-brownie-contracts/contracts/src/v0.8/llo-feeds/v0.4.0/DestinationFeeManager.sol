    ...
    TypeAndVersionInterface
{
     ...
     */
    constructor(address _linkAddress, address _nativeAddress, address _verifierAddress, address _rewardManagerAddress)
    ...
    function processFee(bytes32 recipient, bytes calldata payload, bytes calldata parameterPayload, address subscriber)
        external
        payable
        override
        onlyVerifier
    ...
    function _calculateFee(bytes calldata payload, bytes calldata parameterPayload, address subscriber)
        internal
        view
        returns (Common.Asset memory, Common.Asset memory, uint256)
    {
