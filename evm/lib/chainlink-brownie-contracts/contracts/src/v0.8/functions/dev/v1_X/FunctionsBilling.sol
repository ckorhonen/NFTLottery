...
abstract contract FunctionsBilling is Routable, IFunctionsBilling {
    ...
    // |                       Initialization                         |
    // ================================================================
    constructor(address router, FunctionsBillingConfig memory config, address linkToNativeFeed, address linkToUsdFeed)
        Routable(router)
    {
    ...
    function _startBilling(FunctionsResponse.RequestMeta memory request)
        internal
        returns (FunctionsResponse.Commitment memory commitment, uint72 operationFee)
    {
    ...
    function _fulfillAndBill(
        ...
        bytes memory,
        /* offchainMetadata TODO: use in getDonFee() for dynamic billing */
        ...
        uint256 gasOverheadWei =
            (commitment.gasOverheadBeforeCallback + commitment.gasOverheadAfterCallback) * tx.gasprice;
