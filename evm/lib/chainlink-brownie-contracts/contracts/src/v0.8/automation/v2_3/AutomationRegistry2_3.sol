...
contract AutomationRegistry2_3 is AutomationRegistryBase2_3, OCR2Abstract, Chainable {
     ...
     */
    constructor(AutomationRegistryLogicA2_3 logicA)
    ...
    function _handleReport(HotVars memory hotVars, Report memory report, uint256 gasOverhead) private {
        ...
        for (uint256 i = 0; i < report.upkeepIds.length; i++) {
            ...
            upkeepTransmitInfo[i].calldataWeight = report.performDatas[i].length
...

        {
            ...
            for (uint256 i = 0; i < report.upkeepIds.length; i++) {
                if (upkeepTransmitInfo[i].earlyChecksPassed) {
                    if (
                        i == 0
                            || upkeepTransmitInfo[i].upkeep.billingToken
                                != upkeepTransmitInfo[i - 1].upkeep.billingToken
                    ) {
                        billingTokenParams = _getBillingTokenPaymentParams(
                            hotVars, upkeepTransmitInfo[i].upkeep.billingToken
                        );
                    }
                    PaymentReceipt memory receipt = _handlePayment(
                        ...
                        PaymentParams({
                            ...
                            l1CostWei: (l1Fee * upkeepTransmitInfo[i].calldataWeight)
                                / transmitVars.totalCalldataWeight,
