...
contract AutomationRegistryLogicB2_3 is AutomationRegistryBase2_3, Chainable {
     ...
     */
    constructor(AutomationRegistryLogicC2_3 logicC)
        ...
        )
    {
...

        {
            ...
            if (upkeep.paused) {
                return (false, bytes(""), UpkeepFailureReason.UPKEEP_PAUSED, 0, upkeep.performGas, 0, 0);
            }
            (fastGasWei, linkUSD, nativeUSD) = _getFeedData(hotVars);
            maxPayment = _getMaxPayment(
                id, hotVars, triggerType, upkeep.performGas, fastGasWei, linkUSD, nativeUSD, upkeep.billingToken
            );
