...
contract UpkeepBalanceMonitor is ConfirmedOwner, Pausable {
    ...
    function topUp(uint256[] memory upkeepIDs, address[] memory registryAddresses, uint96[] memory topUpAmounts)
        public
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < upkeepIDs.length; i++) {
            try LINK_TOKEN.transferAndCall(
                registryAddresses[i], topUpAmounts[i], abi.encode(upkeepIDs[i])
