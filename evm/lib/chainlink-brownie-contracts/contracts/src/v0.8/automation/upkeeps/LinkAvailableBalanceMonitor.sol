...
contract LinkAvailableBalanceMonitor is AccessControl, AutomationCompatibleInterface, Pausable {
    ...
    function setWatchList(
        ...
        for (uint256 idx = 0; idx < addresses.length; idx++) {
            ...
            s_targets[targetAddress] = MonitoredAddress({
                isActive: true, minBalance: minBalances[idx], topUpAmount: topUpAmounts[idx], lastTopUpTimestamp: 0
            });
    ...
    function getAccountInfo(address targetAddress)
        external
        view
        returns (bool isActive, uint96 minBalance, uint96 topUpAmount, uint56 lastTopUpTimestamp)
    {
