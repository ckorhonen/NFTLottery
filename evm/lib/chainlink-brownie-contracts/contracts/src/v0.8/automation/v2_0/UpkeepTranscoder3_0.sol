...
contract UpkeepTranscoder3_0 is UpkeepTranscoderInterface, TypeAndVersionInterface {
     ...
     */
    function transcodeUpkeeps(UpkeepFormat fromVersion, UpkeepFormat, bytes calldata encodedUpkeeps)
        external
        view
        override
        returns (bytes memory)
    {
        ...
        if (fromVersion == UpkeepFormat.V1) {
            (uint256[] memory ids, UpkeepV1[] memory upkeepsV1, bytes[] memory checkDatas) =
                abi.decode(encodedUpkeeps, (uint256[], UpkeepV1[], bytes[]));
        ...
        if (fromVersion == UpkeepFormat.V2) {
            (uint256[] memory ids, UpkeepV2[] memory upkeepsV2, bytes[] memory checkDatas) =
                abi.decode(encodedUpkeeps, (uint256[], UpkeepV2[], bytes[]));
