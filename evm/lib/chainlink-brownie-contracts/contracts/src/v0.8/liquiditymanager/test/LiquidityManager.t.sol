...
contract LiquidityManagerSetup is LiquidityManagerBaseTest {
    ...
    function setUp() public virtual override {
        ...
        s_liquidityManager =
            new LiquidityManagerHelper(s_l1Token, i_localChainSelector, s_lockReleaseTokenPool, 0, FINANCE);
        ...
        s_wethRebalancer = new LiquidityManagerHelper(
            IERC20(address(s_l1Weth)), i_localChainSelector, s_wethLockReleaseTokenPool, 0, FINANCE
        );
...
contract LiquidityManager_rebalanceLiquidity is LiquidityManagerSetup {
    ...
    function test_rebalanceBetweenPoolsSuccess() external {
        ...
        LiquidityManager mockRemoteRebalancer =
            new LiquidityManager(s_l1Token, i_remoteChainSelector, mockRemoteBridgeAdapter, 0, FINANCE);
    ...
    function test_rebalanceBetweenPoolsSuccess_AlreadyFinalized() external {
        ...
        LiquidityManager remoteRebalancer =
            new LiquidityManager(s_l2Token, i_remoteChainSelector, remotePool, 0, FINANCE);
        ...
        MockL1BridgeAdapter.Payload memory payload = MockL1BridgeAdapter.Payload({
            action: MockL1BridgeAdapter.FinalizationAction.ProveWithdrawal, data: abi.encode(provePayload)
        });
        bool fundsAvailable =
        ...
        payload = MockL1BridgeAdapter.Payload({
            action: MockL1BridgeAdapter.FinalizationAction.FinalizeWithdrawal, data: abi.encode(finalizePayload)
        });
        fundsAvailable =
            s_bridgeAdapter.finalizeWithdrawERC20(address(0), address(s_liquidityManager), abi.encode(payload));
    ...
    function test_rebalanceBetweenPools_NativeRewrap() external {
        ...
        LiquidityManager remoteRebalancer =
            new LiquidityManager(IERC20(address(s_l2Weth)), i_remoteChainSelector, remotePool, 0, FINANCE);
...
contract LiquidityManager_setLocalLiquidityContainer is LiquidityManagerSetup {
    ...
    function test_setLocalLiquidityContainerSuccess() external {
        LockReleaseTokenPool newPool =
            new LockReleaseTokenPool(s_l1Token, new address[](0), address(1), true, address(123));
