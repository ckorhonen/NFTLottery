...
contract KeystoneForwarder is OwnerIsCreator, ITypeAndVersion, IRouter {
        ...
        bytes2 reportId;
        {
            ...
            for (uint256 i = 0; i < signatures.length; ++i) {
                ...
                address signer = ecrecover(
                    completeHash, uint8(signature[64]) + 27, bytes32(signature[0:32]), bytes32(signature[32:64])
                );
    ...
    function _getMetadata(bytes memory rawReport)
        internal
        pure
        returns (bytes32 workflowExecutionId, uint64 configId, bytes2 reportId)
    {
