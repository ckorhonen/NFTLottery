...
abstract contract KeystoneFeedsPermissionHandler is OwnerIsCreator {
    ...
    error ReportForwarderUnauthorized(
        address forwarder, address workflowOwner, bytes10 workflowName, bytes2 reportName
    );
    ...
    function _setReportPermission(Permission memory permission) internal {
        bytes32 reportId = _createReportId(
            permission.forwarder, permission.workflowOwner, permission.workflowName, permission.reportName
        );
    ...
    function _createReportId(address forwarder, address workflowOwner, bytes10 workflowName, bytes2 reportName)
        internal
        pure
