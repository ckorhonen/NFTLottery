...
contract CapabilitiesRegistry is OwnerIsCreator, ITypeAndVersion {
        ...
        for (uint256 i; i < nodes.length; ++i) {
            ...
            if (nodeWorkflowDONId != 0) {
                ...
                for (uint256 j; j < workflowDonCapabilityIds.length; ++j) {
                    if (!storedNode.supportedHashedCapabilityIds[capabilityConfigCount]
                        .contains(workflowDonCapabilityIds[j])) {
    ...
    function deprecateCapabilities(bytes32[] calldata hashedCapabilityIds) external onlyOwner {
        for (uint256 i; i < hashedCapabilityIds.length; ++i) {
            ...
            if (!s_deprecatedHashedCapabilityIds.add(hashedCapabilityId)) {
    ...
    function getCapabilityConfigs(uint32 donId, bytes32 capabilityId)
        external
        view
    ...
    function _setDONConfig(
        ...
        if (donParams.f == 0 || donParams.f + 1 > nodes.length) {
        ...
        for (uint256 i; i < capabilityConfigurations.length; ++i) {
            ...
            for (uint256 j; j < nodes.length; ++j) {
                if (!s_nodes[nodes[j]].supportedHashedCapabilityIds[s_nodes[nodes[j]].configCount]
                    .contains(configuration.capabilityId)) revert NodeDoesNotSupportCapability(
                    nodes[j], configuration.capabilityId
                );
            }
