...
contract L2EPTest is Test {
    ...
    function encodeCrossDomainSetGreetingMsg(bytes4 selector, address greeterAddr, string memory message)
        public
        pure
        returns (bytes memory)
    {
        return abi.encodeWithSelector(
            selector, greeterAddr, abi.encodeWithSelector(Greeter.setGreeting.selector, message)
        );
    }
    ...
    function encodeCrossDomainMultiSendMsg(bytes4 selector, address multiSendAddr, bytes memory encodedTxs)
        public
        pure
        returns (bytes memory)
    {
        return abi.encodeWithSelector(
            selector, multiSendAddr, abi.encodeWithSelector(MultiSend.multiSend.selector, encodedTxs)
        );
    }
