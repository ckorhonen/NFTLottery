...
contract ChainSpecificUtilTest is BaseTest {
    ...
    function testGetBlockNumberArbitrum() public {
        ...
        for (uint256 i = 0; i < chainIds.length; i++) {
            ...
            uint256 expectedBlockNumber = expectedBlockNumbers[i];
            vm.mockCall(
                ARBSYS_ADDR, abi.encodeWithSelector(ArbSys.arbBlockNumber.selector), abi.encode(expectedBlockNumber)
            );
        ...
        for (uint256 i = 0; i < chainIds.length; i++) {
            ...
            vm.mockCall(
                ...
                abi.encodeWithSelector(
                    OVM_GasPriceOracle.getL1Fee.selector, bytes.concat(someCalldata, L1_FEE_DATA_PADDING)
                ),
