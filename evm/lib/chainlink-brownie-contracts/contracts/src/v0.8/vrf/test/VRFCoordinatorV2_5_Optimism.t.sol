...
contract VRFV2CoordinatorV2_5_Optimism is BaseTest {
    ...
    function _encodeCalculatePaymentAmountNativeExternal(uint256 startGas, uint256 weiPerUnitGas, bool onlyPremium)
        internal
        pure
    ...
    function _encodeCalculatePaymentAmountLinkExternal(uint256 startGas, uint256 weiPerUnitGas, bool onlyPremium)
        internal
        pure
        returns (bytes memory)
    {
    ...
    function _mockGasOraclePriceFeeMethods() internal {
        ...
        vm.mockCall(
            OVM_GASPRICEORACLE_ADDR, abi.encodeWithSelector(bytes4(keccak256("baseFeeScalar()"))), abi.encode(1101)
        );
        vm.mockCall(
            OVM_GASPRICEORACLE_ADDR,
