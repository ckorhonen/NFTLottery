...
contract FeeQuoter is AuthorizedCallers, IFeeQuoter, ITypeAndVersion, IReceiver, KeystoneFeedsPermissionHandler {
    ...
    function getTokenAndGasPrices(address token, uint64 destChainSelector)
        public
        view
        returns (uint224 tokenPrice, uint224 gasPriceValue)
    {
    ...
    function convertTokenAmount(address fromToken, uint256 fromTokenAmount, address toToken)
        public
        view
        returns (uint256)
    {
    ...
    function _getTokenPriceFromDataFeed(TokenPriceFeedConfig memory priceFeedConfig)
        internal
        view
        ...
        returns (uint256 feeTokenAmount)
    {
        ...
        if (numberOfTokens > 0) {
            (premiumFee, tokenTransferGas, tokenTransferBytesOverhead) = _getTokenTransferCost(
                destChainConfig, destChainSelector, message.feeToken, feeTokenPrice, message.tokenAmounts
            );
        } else {
        ...
        uint256 executionCost =
            ...
            * (destChainConfig.destGasOverhead
                + (message.data.length * destChainConfig.destGasPerPayloadByte)
                + tokenTransferGas
    ...
    function applyPremiumMultiplierWeiPerEthUpdates(PremiumMultiplierWeiPerEthArgs[] memory premiumMultiplierWeiPerEthArgs)
        external
        onlyOwner
    {
        _applyPremiumMultiplierWeiPerEthUpdates(premiumMultiplierWeiPerEthArgs);
    ...
    function _applyPremiumMultiplierWeiPerEthUpdates(PremiumMultiplierWeiPerEthArgs[] memory premiumMultiplierWeiPerEthArgs)
        internal
    {
        ...
        returns (uint256 tokenTransferFeeUSDWei, uint32 tokenTransferGas, uint32 tokenTransferBytesOverhead)
    {
        ...
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            ...
            TokenTransferFeeConfig memory transferFeeConfig =
                s_tokenTransferFeeConfig[destChainSelector][tokenAmount.token];
    ...
    function _calculateRebasedValue(uint8 dataFeedDecimal, uint8 tokenDecimal, uint256 feedValue)
        internal
        pure
        returns (uint224 rebasedValue)
    {
    ...
    function _getDataAvailabilityCost(
        ...
        uint256 dataAvailabilityGas =
            (dataAvailabilityLengthBytes * destChainConfig.destGasPerDataAvailabilityByte)
        ...
        return
            ((dataAvailabilityGas * dataAvailabilityGasPrice) * destChainConfig.destDataAvailabilityMultiplierBps)
                * 1e14;
    }
    ...
    function getTokenTransferFeeConfig(uint64 destChainSelector, address token)
        external
        view
    ...
    function _parseEVMExtraArgsFromBytes(bytes calldata extraArgs, DestChainConfig memory destChainConfig)
        internal
        pure
        returns (Client.EVMExtraArgsV2 memory)
    {
    ...
    function _parseUnvalidatedEVMExtraArgsFromBytes(bytes calldata extraArgs, uint64 defaultTxGasLimit)
        private
        pure
        returns (Client.EVMExtraArgsV2 memory)
    {
    ...
    function getStaticConfig() external view returns (StaticConfig memory) {
        return StaticConfig({
            maxFeeJuelsPerMsg: i_maxFeeJuelsPerMsg, linkToken: i_linkToken, stalenessThreshold: i_stalenessThreshold
        });
    }
}
