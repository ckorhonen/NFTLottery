...
contract OffRamp is ITypeAndVersion, MultiOCR3Base {
    ...
    function getExecutionState(uint64 sourceChainSelector, uint64 sequenceNumber)
        public
        view
    ...
    function _getSequenceNumberBitmap(uint64 sourceChainSelector, uint64 sequenceNumber)
        internal
        view
    ...
    function manuallyExecute(Internal.ExecutionReport[] memory reports, GasLimitOverride[][] memory gasLimitOverrides)
        external
    {
            ...
            for (uint256 msgIndex = 0; msgIndex < numMsgs; ++msgIndex) {
                ...
                if (newLimit != 0) {
                    if (newLimit < message.gasLimit) {
                        revert InvalidManualExecutionGasLimit(
                            report.sourceChainSelector, message.header.messageId, newLimit
                        );
                    }
                }
                if (message.tokenAmounts.length != msgGasLimitOverrides[msgIndex].tokenGasOverrides.length) {
                    revert ManualExecutionGasAmountCountMismatch(
                        message.header.messageId, message.header.sequenceNumber
                    );
                }
    ...
    function _executeSingleReport(
        ...
        for (uint256 i = 0; i < numMsgs; ++i) {
            ...
            _setExecutionState(
                sourceChainSelector, message.header.sequenceNumber, Internal.MessageExecutionState.IN_PROGRESS
            );
    ...
    function _getBalanceOfReceiver(address receiver, address token, uint256 gasLimit)
        internal
        returns (uint256 balance, uint256 gasLeft)
    {
    ...
    function commit(
        ...
        if (commitReport.merkleRoots.length > 0) {
            i_rmnRemote.verify(
                address(this), commitReport.merkleRoots, commitReport.rmnSignatures, commitReport.rmnRawVs
            );
        }

        // Check if the report contains price updates
        if (
            commitReport.priceUpdates.tokenPriceUpdates.length > 0
                || commitReport.priceUpdates.gasPriceUpdates.length > 0
        ) {
