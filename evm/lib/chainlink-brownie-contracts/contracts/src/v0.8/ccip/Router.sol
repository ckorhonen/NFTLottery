...
contract Router is IRouter, IRouterClient, ITypeAndVersion, OwnerIsCreator {
    ...
    function _mergeChainSelectorAndOffRamp(uint64 sourceChainSelector, address offRampAddress)
        internal
        pure
        returns (uint256)
    {
    ...
    function getOffRamps() external view returns (OffRamp[] memory) {
        ...
        for (uint256 i = 0; i < encodedOffRamps.length; ++i) {
            ...
            offRamps[i] = OffRamp({
                sourceChainSelector: uint64(encodedOffRamp >> 160), offRamp: address(uint160(encodedOffRamp))
            });
        }
        return offRamps;
    }
