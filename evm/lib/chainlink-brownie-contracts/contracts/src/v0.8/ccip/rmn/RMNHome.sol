...
contract RMNHome is OwnerIsCreator, ITypeAndVersion {
    event ConfigSet(
        bytes32 indexed configDigest, uint32 version, StaticConfig staticConfig, DynamicConfig dynamicConfig
    );
        ...
        return bytes32(
            (PREFIX & PREFIX_MASK)
                | (uint256(
                        keccak256(
                            bytes.concat(
                                abi.encode(bytes32("EVM"), block.chainid, address(this), version), staticConfig
                            )
                        )
                    )
                    & ~PREFIX_MASK)
    ...
    /// @param staticConfig The static part of the config.
    /// @param dynamicConfig The dynamic part of the config.
    function _validateStaticAndDynamicConfig(StaticConfig memory staticConfig, DynamicConfig memory dynamicConfig)
        internal
        pure
    {
