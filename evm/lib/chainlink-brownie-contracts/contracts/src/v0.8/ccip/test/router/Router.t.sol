...
contract Router_applyRampUpdates is RouterSetup {
    ...
    function _assertOffRampRouteSucceeds(Router.OffRamp memory offRamp) internal {
        ...
        vm.expectCall(
            address(s_receiver), abi.encodeWithSelector(IAny2EVMMessageReceiver.ccipReceive.selector, message)
        );
    ...
    function _assertOffRampRouteReverts(Router.OffRamp memory offRamp) internal {
        ...
        s_sourceRouter.routeMessage(
            _generateReceiverMessage(offRamp.sourceChainSelector),
    ...
    function test_OffRampUpdatesWithRouting() public {
        ...
        for (uint256 i = 0; i < numberOfPartialUpdates; ++i) {
            ...
            assertTrue(
                s_sourceRouter.isOffRamp(partialOffRampAdds[i].sourceChainSelector, partialOffRampAdds[i].offRamp)
            );
        ...
        for (uint256 i = 0; i < numberOfRemainingOfframps; ++i) {
            vm.expectEmit();
            emit Router.OffRampRemoved(
                remainingOffRampRemoves[i].sourceChainSelector, remainingOffRampRemoves[i].offRamp
            );
        }
