...
contract FeeQuoterSetup is TokenSetup {
    ...
    function _getPriceUpdatesStruct(address[] memory tokens, uint224[] memory prices)
        internal
        pure
        returns (Internal.PriceUpdates memory)
    {
        ...
        Internal.PriceUpdates memory priceUpdates = Internal.PriceUpdates({
            tokenPriceUpdates: tokenPriceUpdates, gasPriceUpdates: new Internal.GasPriceUpdate[](0)
        });
    ...
    function _getSingleTokenPriceFeedUpdateStruct(address sourceToken, address dataFeedAddress, uint8 tokenDecimals)
        internal
        pure
        returns (FeeQuoter.TokenPriceFeedUpdate memory)
    {
    ...
    function _generateTokenTransferFeeConfigArgs(uint256 destChainSelectorLength, uint256 tokenLength)
        internal
        pure
        returns (FeeQuoter.TokenTransferFeeConfigArgs[] memory)
    {
    ...
    function _assertTokenPriceFeedConfigUnconfigured(FeeQuoter.TokenPriceFeedConfig memory config)
        internal
        pure
        virtual
    {
    ...
    function _assertFeeQuoterStaticConfigsEqual(FeeQuoter.StaticConfig memory a, FeeQuoter.StaticConfig memory b)
        internal
        pure
    {
