...
contract OffRampSetup is FeeQuoterSetup, MultiOCR3BaseSetup {
    ...
    function _setupMultipleOffRampsFromConfigs(OffRamp.SourceChainConfigArgs[] memory sourceChainConfigs) internal {
        ...
        for (uint256 i = 0; i < sourceChainConfigs.length; ++i) {
            ...
            offRampUpdates[2 * i] =
                Router.OffRamp({sourceChainSelector: sourceChainSelector, offRamp: address(s_offRamp)});
    ...
    function _generateAny2EVMMessageNoTokens(uint64 sourceChainSelector, bytes memory onRamp, uint64 sequenceNumber)
        internal
        view
        returns (Internal.Any2EVMRampMessage memory)
    {
        return
            _generateAny2EVMMessage(sourceChainSelector, onRamp, sequenceNumber, new Client.EVMTokenAmount[](0), false);
    }
    ...
    function _generateSingleBasicMessage(uint64 sourceChainSelector, bytes memory onRamp)
        internal
        view
        returns (Internal.Any2EVMRampMessage[] memory)
    {
    ...
    function _generateMessagesWithTokens(uint64 sourceChainSelector, bytes memory onRamp)
        internal
        view
        returns (Internal.Any2EVMRampMessage[] memory)
    {
    ...
    function _generateReportFromMessages(uint64 sourceChainSelector, Internal.Any2EVMRampMessage[] memory messages)
        internal
        pure
    ...
    function _getDefaultSourceTokenData(Client.EVMTokenAmount[] memory srcTokenAmounts)
        internal
        view
        returns (Internal.Any2EVMTokenTransfer[] memory)
    {
        Internal.Any2EVMTokenTransfer[] memory sourceTokenData =
            new Internal.Any2EVMTokenTransfer[](srcTokenAmounts.length);
    ...
    function _hashMessage(Internal.Any2EVMRampMessage memory message, bytes memory onRamp)
        internal
        pure
        returns (bytes32)
    {
