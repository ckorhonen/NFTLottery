...
contract FeeQuoter_constructor is FeeQuoterSetup {
    ...
    function test_InvalidStalenessThreshold_Revert() public {
        FeeQuoter.StaticConfig memory staticConfig = FeeQuoter.StaticConfig({
            linkToken: s_sourceTokens[0], maxFeeJuelsPerMsg: MAX_MSG_FEES_JUELS, stalenessThreshold: 0
        });
    ...
    function test_InvalidLinkTokenEqZeroAddress_Revert() public {
        FeeQuoter.StaticConfig memory staticConfig = FeeQuoter.StaticConfig({
            linkToken: address(0), maxFeeJuelsPerMsg: MAX_MSG_FEES_JUELS, stalenessThreshold: uint32(TWELVE_HOURS)
        });
    ...
    function test_InvalidMaxFeeJuelsPerMsg_Revert() public {
        FeeQuoter.StaticConfig memory staticConfig = FeeQuoter.StaticConfig({
            linkToken: s_sourceTokens[0], maxFeeJuelsPerMsg: 0, stalenessThreshold: uint32(TWELVE_HOURS)
        });
...
contract FeeQuoter_updatePrices is FeeQuoterSetup {
    ...
    function test_UpdateMultiplePrices_Success() public {
        ...
        for (uint256 i = 0; i < tokenPriceUpdates.length; ++i) {
            assertEq(
                s_feeQuoter.getTokenPrice(update.tokenPriceUpdates[i].sourceToken).value,
    ...
    function test_UpdatableByAuthorizedCaller_Success() public {
        ...
        priceUpdates.tokenPriceUpdates[0] =
            Internal.TokenPriceUpdate({sourceToken: s_sourceTokens[0], usdPerToken: 4e18});
        ...
        vm.expectEmit();
        emit FeeQuoter.UsdPerTokenUpdated(
            priceUpdates.tokenPriceUpdates[0].sourceToken,
            priceUpdates.tokenPriceUpdates[0].usdPerToken,
            block.timestamp
        );
        s_feeQuoter.updatePrices(priceUpdates);
...
contract FeeQuoter_convertTokenAmount is FeeQuoterSetup {
    function test_ConvertTokenAmount_Success() public view {
        Internal.PriceUpdates memory initialPriceUpdates =
            abi.decode(s_encodedInitialPriceUpdates, (Internal.PriceUpdates));
        uint256 amount = 3e16;
        uint256 conversionRate =
            (uint256(initialPriceUpdates.tokenPriceUpdates[2].usdPerToken) * 1e18)
    ...
    function test_Fuzz_ConvertTokenAmount_Success(
        ...
        gasPriceUpdates[0] =
            Internal.GasPriceUpdate({destChainSelector: DEST_CHAIN_SELECTOR, usdPerUnitGas: usdPerUnitGas});
...
contract FeeQuoter_getTokenAndGasPrices is FeeQuoterSetup {
    function test_GetFeeTokenAndGasPrices_Success() public view {
        (uint224 feeTokenPrice, uint224 gasPrice) =
            s_feeQuoter.getTokenAndGasPrices(s_sourceFeeToken, DEST_CHAIN_SELECTOR);
    ...
    function test_ZeroGasPrice_Success() public {
        ...
        Internal.PriceUpdates memory priceUpdates = Internal.PriceUpdates({
            tokenPriceUpdates: new Internal.TokenPriceUpdate[](0), gasPriceUpdates: gasPriceUpdates
        });
        s_feeQuoter.updatePrices(priceUpdates);
    ...
    function test_StaleGasPrice_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(FeeQuoter.StaleGasPrice.selector, DEST_CHAIN_SELECTOR, TWELVE_HOURS, diff)
        );
...
contract FeeQuoter_updateTokenPriceFeeds is FeeQuoterSetup {
    ...
    function test_SingleFeedUpdate_Success() public {
        ...
        _assertTokenPriceFeedConfigUnconfigured(
            s_feeQuoter.getTokenPriceFeedConfig(tokenPriceFeedUpdates[0].sourceToken)
        );

        vm.expectEmit();
        emit FeeQuoter.PriceFeedPerTokenUpdated(
    ...
    function test_MultipleFeedUpdate_Success() public {
        ...
        _assertTokenPriceFeedConfigEquality(
            s_feeQuoter.getTokenPriceFeedConfig(tokenPriceFeedUpdates[1].sourceToken),
...
contract FeeQuoter_applyDestChainConfigUpdates is FeeQuoterSetup {
        ...
        public
    {
        ...
        if (isNewChain) {
            ...
            vm.expectEmit();
            emit FeeQuoter.DestChainConfigUpdated(
...
contract FeeQuoter_getDataAvailabilityCost is FeeQuoterSetup {
    function test_EmptyMessageCalculatesDataAvailabilityCost_Success() public {
        ...
        uint256 dataAvailabilityGas =
            destChainConfig.destDataAvailabilityOverheadGas + destChainConfig.destGasPerDataAvailabilityByte
            * Internal.MESSAGE_FIXED_BYTES;
        uint256 expectedDataAvailabilityCostUSD =
            USD_PER_DATA_AVAILABILITY_GAS * dataAvailabilityGas * destChainConfig.destDataAvailabilityMultiplierBps
            * 1e14;
    ...
    function test_SimpleMessageCalculatesDataAvailabilityCostUnsupportedDestChainSelector_Success() public view {
        uint256 dataAvailabilityCostUSD =
            s_feeQuoter.getDataAvailabilityCost(0, USD_PER_DATA_AVAILABILITY_GAS, 100, 5, 50);
    ...
    function test_Fuzz_CalculateDataAvailabilityCost_Success(
        ...
        uint256 dataAvailabilityLengthBytes =
            Internal.MESSAGE_FIXED_BYTES + messageDataLength + (numberOfTokens * Internal.MESSAGE_FIXED_BYTES_PER_TOKEN)
            + tokenTransferBytesOverhead;
...
contract FeeQuoter_applyTokenTransferFeeConfigUpdates is FeeQuoterSetup {
    function test_Fuzz_ApplyTokenTransferFeeConfig_Success(FeeQuoter
                .TokenTransferFeeConfig[2] memory tokenTransferFeeConfigs) public {
        ...
        for (uint256 i = 0; i < tokenTransferFeeConfigArgs.length; ++i) {
            for (uint256 j = 0; j < tokenTransferFeeConfigs.length; ++j) {
                tokenTransferFeeConfigs[j].destBytesOverhead = uint32(
                    bound(
                        tokenTransferFeeConfigs[j].destBytesOverhead,
                ...
                tokenTransferFeeConfigArgs[i].tokenTransferFeeConfigs[j].tokenTransferFeeConfig =
...
contract FeeQuoter_getTokenTransferCost is FeeQuoterFeeSetup {
    ...
    function test_NoTokenTransferChargesZeroFee_Success() public view {
        ...
        (uint256 feeUSDWei, uint32 destGasOverhead, uint32 destBytesOverhead) = s_feeQuoter.getTokenTransferCost(
            DEST_CHAIN_SELECTOR, message.feeToken, s_feeTokenPrice, message.tokenAmounts
        );
    ...
    function test_MixedTokenTransferFee_Success() public view {
        ...
        uint256 token0USDWei = _applyBpsRatio(
            _calcUSDValueFromTokenAmount(tokenPrices[0], message.tokenAmounts[0].amount),
...
contract FeeQuoter_getValidatedFee is FeeQuoterFeeSetup {
    ...
    function test_EmptyMessage_Success() public view {
        ...
        for (uint256 i = 0; i < feeTokenPrices.length; ++i) {
            ...
            uint256 messageFeeUSD =
                (_configUSDCentToWei(destChainConfig.networkFeeUSDCents) * premiumMultiplierWeiPerEth);
    ...
    function test_SingleTokenMessage_Success() public view {
        ...
        for (uint256 i = 0; i < feeTokenPrices.length; ++i) {
            ...
            uint256 gasUsed =
                GAS_LIMIT + DEST_GAS_OVERHEAD
                + s_feeQuoter.getTokenTransferFeeConfig(DEST_CHAIN_SELECTOR, message.tokenAmounts[0].token)
                .destGasOverhead;
            uint256 gasFeeUSD = (gasUsed * destChainConfig.gasMultiplierWeiPerEth * USD_PER_GAS);
            (uint256 transferFeeUSD,,) = s_feeQuoter.getTokenTransferCost(
    ...
    function test_MessageWithDataAndTokenTransfer_Success() public view {
        ...
        for (uint256 i = 0; i < feeTokenPrices.length; ++i) {
            ...
            for (uint256 j = 0; j < message.tokenAmounts.length; ++j) {
                tokenGasOverhead += s_feeQuoter.getTokenTransferFeeConfig(
                    DEST_CHAIN_SELECTOR, message.tokenAmounts[j].token
                ).destGasOverhead;
                uint32 destBytesOverhead =
                    s_feeQuoter.getTokenTransferFeeConfig(DEST_CHAIN_SELECTOR, message.tokenAmounts[j].token)
                .destBytesOverhead;
                tokenBytesOverhead += destBytesOverhead == 0
...
contract FeeQuoter_processMessageArgs is FeeQuoterFeeSetup {
    ...
    function test_processMessageArgs_WithEmptyEVMExtraArgs_Success() public view {
        ...
        assertEq(
            convertedExtraArgs, Client._argsToBytes(s_feeQuoter.parseEVMExtraArgsFromBytes("", DEST_CHAIN_SELECTOR))
        );
    }
    ...
    function test_processMessageArgs_WithEVMExtraArgsV1_Success() public view {
        ...
        assertEq(
            convertedExtraArgs,
            Client._argsToBytes(s_feeQuoter.parseEVMExtraArgsFromBytes(extraArgs, DEST_CHAIN_SELECTOR))
        );
    }

    function test_processMessageArgs_WitEVMExtraArgsV2_Success() public view {
        bytes memory extraArgs =
            Client._argsToBytes(Client.EVMExtraArgsV2({gasLimit: 0, allowOutOfOrderExecution: true}));
    ...
    function test_processMessageArgs_WithCorrectPoolReturnData_Success() public view {
        ...
        (/* msgFeeJuels */,/* isOutOfOrderExecution */,/* convertedExtraArgs */, bytes[] memory destExecData) = s_feeQuoter.processMessageArgs(
...
contract FeeQuoter_KeystoneSetup is FeeQuoterSetup {
    ...
    function setUp() public virtual override {
        ...
        KeystoneFeedsPermissionHandler.Permission[] memory permissions =
            new KeystoneFeedsPermissionHandler.Permission[](1);
...
contract FeeQuoter_onReport is FeeQuoter_KeystoneSetup {
    function test_onReport_Success() public {
        ...
        report[0] = FeeQuoter.ReceivedCCIPFeedReport({
            token: onReportTestToken1, price: 4e18, timestamp: uint32(block.timestamp)
        });
        report[1] = FeeQuoter.ReceivedCCIPFeedReport({
            token: onReportTestToken2, price: 4e18, timestamp: uint32(block.timestamp)
        });
    ...
    function test_onReport_InvalidForwarder_Reverts() public {
        ...
        report[0] = FeeQuoter.ReceivedCCIPFeedReport({
            token: s_sourceTokens[0], price: 4e18, timestamp: uint32(block.timestamp)
        });
    ...
    function test_onReport_UnsupportedToken_Reverts() public {
        ...
        FeeQuoter.ReceivedCCIPFeedReport[] memory report = new FeeQuoter.ReceivedCCIPFeedReport[](1);
        report[0] = FeeQuoter.ReceivedCCIPFeedReport({
            token: s_sourceTokens[1], price: 4e18, timestamp: uint32(block.timestamp)
        });
    ...
    function test_OnReport_StaleUpdate_Revert() public {
        ...
        report[0] = FeeQuoter.ReceivedCCIPFeedReport({
            token: onReportTestToken1, price: 4e18, timestamp: uint32(block.timestamp - 1)
        });
        //expecting a revert
        vm.expectRevert(
            abi.encodeWithSelector(
