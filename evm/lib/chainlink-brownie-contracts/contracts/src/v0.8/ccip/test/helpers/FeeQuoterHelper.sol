...
contract FeeQuoterHelper is FeeQuoter {
    ...
    function parseEVMExtraArgsFromBytes(bytes calldata extraArgs, uint64 destChainSelector)
        external
        view
    ...
    function parseEVMExtraArgsFromBytes(bytes calldata extraArgs, DestChainConfig memory destChainConfig)
        external
        pure
        returns (Client.EVMExtraArgsV2 memory)
    {
    ...
    function calculateRebasedValue(uint8 dataFeedDecimal, uint8 tokenDecimal, uint256 feedValue)
        external
        pure
        returns (uint224)
    {
