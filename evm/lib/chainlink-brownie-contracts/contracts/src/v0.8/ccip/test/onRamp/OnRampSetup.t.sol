...
contract OnRampSetup is FeeQuoterFeeSetup {
    ...
    function _generateDestChainConfigArgs(IRouter router) internal pure returns (OnRamp.DestChainConfigArgs[] memory) {
        ...
        destChainConfigs[0] = OnRamp.DestChainConfigArgs({
            destChainSelector: DEST_CHAIN_SELECTOR, router: router, allowListEnabled: false
        });
    ...
    function _deployOnRamp(
        ...
        return (
            onRamp,
            keccak256(
                abi.encode(Internal.EVM_2_ANY_MESSAGE_HASH, sourceChainSelector, DEST_CHAIN_SELECTOR, address(onRamp))
            )
        );
    }
