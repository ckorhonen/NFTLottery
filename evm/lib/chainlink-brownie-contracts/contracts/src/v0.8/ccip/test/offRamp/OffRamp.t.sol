...
contract OffRamp_constructor is OffRampSetup {
    function test_Constructor_Success() public {
        ...
        OffRamp.SourceChainConfig memory expectedSourceChainConfig1 = OffRamp.SourceChainConfig({
            router: s_destRouter, isEnabled: true, minSeqNr: 1, onRamp: sourceChainConfigs[0].onRamp
        });

        OffRamp.SourceChainConfig memory expectedSourceChainConfig2 = OffRamp.SourceChainConfig({
            router: s_destRouter, isEnabled: true, minSeqNr: 1, onRamp: sourceChainConfigs[1].onRamp
        });
        ...
        MultiOCR3Base.OCRConfig memory gotOCRConfig =
            s_offRamp.latestConfigDetails(uint8(Internal.OCRPluginType.Execution));
    ...
    function test_ZeroOnRampAddress_Revert() public {
        ...
        sourceChainConfigs[0] = OffRamp.SourceChainConfigArgs({
            router: s_destRouter, sourceChainSelector: SOURCE_CHAIN_SELECTOR_1, onRamp: new bytes(0), isEnabled: true
        });
    ...
    function test_SourceChainSelector_Revert() public {
        ...
        OffRamp.SourceChainConfigArgs[] memory sourceChainConfigs = new OffRamp.SourceChainConfigArgs[](1);
        sourceChainConfigs[0] = OffRamp.SourceChainConfigArgs({
            router: s_destRouter, sourceChainSelector: 0, onRamp: ON_RAMP_ADDRESS_1, isEnabled: true
        });
...
contract OffRamp_executeSingleReport is OffRampSetup {
    ...
    function test_Fuzz_InterleavingOrderedAndUnorderedMessages_Success(bool[7] memory orderings) public {
        ...
        for (uint256 i = 0; i < orderings.length; ++i) {
            messages[i] = _generateAny2EVMMessage(
                SOURCE_CHAIN_SELECTOR_1, ON_RAMP_ADDRESS_1, uint64(i + 1), tokenAmounts, !orderings[i]
            );
            if (orderings[i]) {
        ...
        assertEq(
            nonceBefore + expectedNonce,
            s_inboundNonceManager.getInboundNonce(SOURCE_CHAIN_SELECTOR_1, abi.encode(OWNER))
        );
    }
    ...
    function test_MismatchingDestChainSelector_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                OffRamp.InvalidMessageDestChainSelector.selector, messages[0].header.destChainSelector
            )
        );
...
contract OffRamp_manuallyExecute is OffRampSetup {
    ...
    function test_manuallyExecute_Success() public {
        ...
        s_offRamp.manuallyExecute(
            _generateBatchReportFromMessages(SOURCE_CHAIN_SELECTOR_1, messages), gasLimitOverrides
        );
        assertExecutionStateChangedEventLogs(
            SOURCE_CHAIN_SELECTOR_1,
            messages[0].header.sequenceNumber,
    ...
    function test_manuallyExecute_WithPartialMessages_Success() public {
        ...
        vm.recordLogs();
        s_offRamp.manuallyExecute(
            _generateBatchReportFromMessages(SOURCE_CHAIN_SELECTOR_1, newMessages), gasLimitOverrides
        );
        assertExecutionStateChangedEventLogs(
            SOURCE_CHAIN_SELECTOR_1,
            messages[0].header.sequenceNumber,
    ...
    function test_manuallyExecute_DestinationGasAmountCountMismatch_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                OffRamp.ManualExecutionGasAmountCountMismatch.selector, messages[0].header.messageId, 1
            )
        );
        s_offRamp.manuallyExecute(
            _generateBatchReportFromMessages(SOURCE_CHAIN_SELECTOR_1, messages), gasLimitOverrides
...
contract OffRamp_getExecutionState is OffRampSetup {
    ...
    function test_GetExecutionState_Success() public {
        ...
        assertEq(
            ...
            uint256(s_offRamp.getExecutionState(SOURCE_CHAIN_SELECTOR_1, 0))
        );
        assertEq(
        ...
        assertEq(
            uint256(Internal.MessageExecutionState.FAILURE),
            uint256(s_offRamp.getExecutionState(SOURCE_CHAIN_SELECTOR_1, 2))
        );
        assertEq(
            uint256(Internal.MessageExecutionState.IN_PROGRESS),
...
contract OffRamp_releaseOrMintSingleToken is OffRampSetup {
    ...
    function test__releaseOrMintSingleToken_Success() public {
        ...
        s_offRamp.releaseOrMintSingleToken(
            tokenAmount, originalSender, OWNER, SOURCE_CHAIN_SELECTOR_1, offchainTokenData
        );
    ...
    function test_releaseOrMintToken_InvalidDataLength_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(OffRamp.InvalidDataLength.selector, Internal.MAX_BALANCE_OF_RET_BYTES, 64)
        );
...
contract OffRamp_releaseOrMintTokens is OffRampSetup {
    ...
    function test_releaseOrMintTokens_InvalidDataLengthReturnData_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(OffRamp.InvalidDataLength.selector, Pool.CCIP_LOCK_OR_BURN_V1_RET_BYTES, 64)
        );
    ...
    function test_Fuzz__releaseOrMintTokens_AnyRevertIsCaught_Success(address destPool) public {
        ...
        try s_offRamp.releaseOrMintTokens(
            ...
            assertTrue(
                bytes4(reason) == OffRamp.TokenHandlingError.selector
...
contract OffRamp_applySourceChainConfigUpdates is OffRampSetup {
    ...
    function test_AddNewChain_Success() public {
        ...
        _assertSourceChainConfigEquality(
            s_offRamp.getSourceChainConfig(SOURCE_CHAIN_SELECTOR_1), expectedSourceChainConfig
        );
    }
...
contract OffRamp_commit is OffRampSetup {
    ...
    function test_ReportAndPriceUpdate_Success() public {
        ...
        vm.expectEmit();
        emit MultiOCR3Base.Transmitted(
            uint8(Internal.OCRPluginType.Commit), s_configDigestCommit, s_latestSequenceNumber
        );
    ...
    function test_ReportOnlyRootSuccess_gas() public {
        ...
        OffRamp.CommitReport memory commitReport = OffRamp.CommitReport({
            priceUpdates: _getEmptyPriceUpdates(), merkleRoots: roots, rmnSignatures: s_rmnSignatures, rmnRawVs: 0
        });
    ...
    function test_StaleReportWithRoot_Success() public {
        ...
        uint224 tokenStartPrice =
            IFeeQuoter(s_offRamp.getDynamicConfig().feeQuoter).getTokenPrice(s_sourceFeeToken).value;
        ...
        assertEq(0, s_offRamp.getLatestPriceSequenceNumber());
        assertEq(
            tokenStartPrice, IFeeQuoter(s_offRamp.getDynamicConfig().feeQuoter).getTokenPrice(s_sourceFeeToken).value
        );
    }
    ...
    function test_RootAlreadyCommitted_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                OffRamp.RootAlreadyCommitted.selector, roots[0].sourceChainSelector, roots[0].merkleRoot
            )
        );
        _commit(commitReport, ++s_latestSequenceNumber);
    }
    ...
    function test_CommitOnRampMismatch_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(OffRamp.CommitOnRampMismatch.selector, ON_RAMP_ADDRESS_2, ON_RAMP_ADDRESS_1)
        );
        _commit(commitReport, s_latestSequenceNumber);
    }
