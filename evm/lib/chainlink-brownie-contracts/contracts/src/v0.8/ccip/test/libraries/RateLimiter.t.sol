...
contract RateLimiter_setTokenBucketConfig is RateLimiterSetup {
    function test_SetRateLimiterConfig_Success() public {
        ...
        s_config = RateLimiter.Config({
            isEnabled: true, rate: uint128(rateLimiter.rate * 2), capacity: rateLimiter.capacity * 8
        });
...
contract RateLimiter_consume is RateLimiterSetup {
    ...
    function test_ConsumingMoreThanUint128_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RateLimiter.AggregateValueMaxCapacityExceeded.selector, rateLimiter.capacity, request
            )
        );
    ...
    function test_RateLimitReachedOverConsecutiveBlocks_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(RateLimiter.AggregateValueRateLimitReached.selector, 1, rateLimiter.rate)
        );
