...
abstract contract MultiTokenPool is IPoolV1, OwnerIsCreator {
    ...
    function _validateReleaseOrMint(Pool.ReleaseOrMintInV1 memory releaseOrMintIn) internal {
        ...
        bytes memory configuredRemotePool =
            getRemotePool(releaseOrMintIn.localToken, releaseOrMintIn.remoteChainSelector);
        if (
        ...
        _consumeInboundRateLimit(
            releaseOrMintIn.localToken, releaseOrMintIn.remoteChainSelector, releaseOrMintIn.amount
        );
    }
    ...
    function setRemotePool(address token, uint64 remoteChainSelector, bytes calldata remotePoolAddress)
        external
        onlyOwner
    {
