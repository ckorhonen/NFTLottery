...
contract RMNHomeTest is Test {
    ...
    function _getBaseConfig() internal pure returns (Config memory) {
        ...
        sourceChains[1] =
            RMNHome.SourceChain({chainSelector: 9001, minObservers: 2, observerNodesBitmap: 1 << 1 | 1 << 2});

        return Config({
            staticConfig: RMNHome.StaticConfig({nodes: nodes, offchainConfig: abi.encode("static_config")}),
            dynamicConfig: RMNHome.DynamicConfig({
                sourceChains: sourceChains, offchainConfig: abi.encode("dynamic_config")
            })
        });
    }
        ...
        return bytes32(
            (PREFIX & PREFIX_MASK)
                | (uint256(
                        keccak256(
                            bytes.concat(
                                abi.encode(bytes32("EVM"), block.chainid, address(s_rmnHome), version), staticConfig
                            )
                        )
                    )
                    & ~PREFIX_MASK)
...
contract RMNHome_setCandidate is RMNHomeTest {
    function test_setCandidate_success() public {
        ...
        versionedConfig.configDigest =
            _getConfigDigest(abi.encode(versionedConfig.staticConfig), versionedConfig.version);

        vm.expectEmit();
        emit RMNHome.ConfigSet(
            versionedConfig.configDigest,
            versionedConfig.version,
            versionedConfig.staticConfig,
            versionedConfig.dynamicConfig
        );
        ...
        (RMNHome.VersionedConfig memory storedVersionedConfig, bool ok) =
            s_rmnHome.getConfig(versionedConfig.configDigest);
        assertTrue(ok);
        ...
        for (uint256 i = 0; i < storedDynamicConfig.sourceChains.length; i++) {
            ...
            assertEq(
                storedSourceChain.observerNodesBitmap, versionedConfig.dynamicConfig.sourceChains[i].observerNodesBitmap
            );
        }
...
contract RMNHome_revokeCandidate is RMNHomeTest {
    ...
    function test_revokeCandidate_ConfigDigestMismatch_reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(RMNHome.ConfigDigestMismatch.selector, priorCandidateDigest, wrongDigest)
        );
...
contract RMNHome_promoteCandidateAndRevokeActive is RMNHomeTest {
    ...
    function test_promoteCandidateAndRevokeActive_ConfigDigestMismatch_reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(RMNHome.ConfigDigestMismatch.selector, priorActiveDigest, wrongActiveDigest)
        );
