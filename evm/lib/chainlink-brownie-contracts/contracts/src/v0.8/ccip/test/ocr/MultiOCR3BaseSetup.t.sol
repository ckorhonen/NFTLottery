...
contract MultiOCR3BaseSetup is BaseTest {
    ...
    function _getBasicConfigDigest(uint8 F, address[] memory signers, address[] memory transmitters)
        internal
        view
        returns (bytes32)
    {
    ...
    function _assertOCRConfigEquality(MultiOCR3Base.OCRConfig memory configA, MultiOCR3Base.OCRConfig memory configB)
        internal
        pure
    {
        ...
        vm.assertEq(
            configA.configInfo.isSignatureVerificationEnabled, configB.configInfo.isSignatureVerificationEnabled
        );
