...
contract NonceManager_applyPreviousRampsUpdates is OnRampSetup {
    function test_SingleRampUpdate() public {
        ...
        _assertPreviousRampsEqual(
            s_outboundNonceManager.getPreviousRamps(DEST_CHAIN_SELECTOR), previousRamps[0].prevRamps
        );
    }
    ...
    function test_MultipleRampsUpdates() public {
        ...
        previousRamps[1] = NonceManager.PreviousRampsArgs(
            DEST_CHAIN_SELECTOR + 1, NonceManager.PreviousRamps(prevOnRamp2, prevOffRamp2)
        );
    ...
    function _assertPreviousRampsEqual(NonceManager.PreviousRamps memory a, NonceManager.PreviousRamps memory b)
        internal
        pure
    {
...
contract NonceManager_OnRampUpgrade is OnRampSetup {
    ...
    function setUp() public virtual override {
        ...
        vm.mockCall(
            address(s_prevOnRamp), abi.encodeWithSelector(IEVM2AnyOnRamp.getSenderNonce.selector), abi.encode(0)
        );

        NonceManager.PreviousRampsArgs[] memory previousRamps = new NonceManager.PreviousRampsArgs[](1);
        previousRamps[0] = NonceManager.PreviousRampsArgs(
            DEST_CHAIN_SELECTOR, NonceManager.PreviousRamps(address(s_prevOnRamp), address(0))
        );
        s_outboundNonceManager.applyPreviousRampsUpdates(previousRamps);
    ...
    function test_UpgradeNonceStartsAtV1Nonce_Success() public {
        ...
        emit OnRamp.CCIPMessageSent(
            DEST_CHAIN_SELECTOR, 1, _messageToEvent(message, 1, startNonce + 2, FEE_AMOUNT, OWNER)
        );
        s_onRamp.forwardFromRouter(DEST_CHAIN_SELECTOR, message, FEE_AMOUNT, OWNER);
        ...
        vm.expectEmit();
        emit OnRamp.CCIPMessageSent(
            DEST_CHAIN_SELECTOR, 2, _messageToEvent(message, 2, startNonce + 3, FEE_AMOUNT, OWNER)
        );
        s_onRamp.forwardFromRouter(DEST_CHAIN_SELECTOR, message, FEE_AMOUNT, OWNER);
...
contract NonceManager_OffRampUpgrade is OffRampSetup {
    ...
    function test_NoPrevOffRampForChain_Success() public {
        ...
        assertEq(
            startNonceChain3, s_inboundNonceManager.getInboundNonce(SOURCE_CHAIN_SELECTOR_3, abi.encode(senders[0]))
        );
    ...
    function test_UpgradedSenderNoncesReadsPreviousRamp_Success() public {
        ...
        for (uint64 i = 1; i < 4; ++i) {
            ...
            assertEq(
                startNonce + i, s_inboundNonceManager.getInboundNonce(SOURCE_CHAIN_SELECTOR_1, abi.encode(senders[0]))
            );
        }
    }
