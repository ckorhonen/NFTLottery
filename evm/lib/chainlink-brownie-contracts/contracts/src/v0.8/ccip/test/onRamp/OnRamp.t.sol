...
contract OnRamp_forwardFromRouter is OnRampSetup {
    ...
    function test_ForwardFromRouterSuccessLegacyExtraArgs() public {
        ...
        message.extraArgs = abi.encodeWithSelector(
            Client.EVM_EXTRA_ARGS_V1_TAG, LegacyExtraArgs({gasLimit: GAS_LIMIT * 2, strict: true})
        );
        uint256 feeAmount = 1234567890;
        IERC20(s_sourceFeeToken).transferFrom(OWNER, address(s_onRamp), feeAmount);
    ...
    function test_ShouldStoreNonLinkFees() public {
        ...
        emit OnRamp.CCIPMessageSent(
            DEST_CHAIN_SELECTOR, 1, _messageToEvent(message, 1, 1, feeAmount, expectedJuels, OWNER)
        );
    ...
    function test_Fuzz_ForwardFromRouter_Success(address originalSender, address receiver, uint96 feeTokenAmount)
        public
    {
        ...
        Internal.EVM2AnyRampMessage memory expectedEvent =
...
contract OnRamp_getFee is OnRampSetup {
    ...
    function test_GetFeeOfZeroForTokenMessage_Success() public {
        ...
        tokenMults[0] =
            FeeQuoter.PremiumMultiplierWeiPerEthArgs({token: message.feeToken, premiumMultiplierWeiPerEth: 0});
...
contract OnRamp_applyDestChainConfigUpdates is OnRampSetup {
    function test_ApplyDestChainConfigUpdates_Success() external {
        ...
        configArgs[1] = OnRamp.DestChainConfigArgs({
            destChainSelector: 9999, router: IRouter(address(9999)), allowListEnabled: false
        });
        vm.expectEmit();
        emit OnRamp.DestChainConfigSet(DEST_CHAIN_SELECTOR, 0, s_sourceRouter, false);
        vm.expectEmit();
