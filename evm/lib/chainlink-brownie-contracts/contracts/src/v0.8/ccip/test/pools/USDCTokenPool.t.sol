    ...
    function test_Fuzz_SetDomains_Success(
        ...
        for (uint256 i = 0; i < numberOfDomains; ++i) {
            ...
            s_chainToDomain[destChainSelectors[i]] = USDCTokenPool.Domain({
                domainIdentifier: domainIdentifiers[i], allowedCaller: allowedCallers[i], enabled: true
            });
        }
    ...
    function test_ValidateInvalidMessage_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                USDCTokenPool.InvalidSourceDomain.selector, expectedSourceDomain, usdcMessage.sourceDomain
            )
        );
        s_usdcTokenPool.validateMessage(
            encodedUsdcMessage,
