...
contract TokenPool_constructor is TokenPoolSetup {
    ...
    function test_ZeroAddressNotAllowed_Revert() public {
        ...
        s_tokenPool =
            new TokenPoolHelper(IERC20(address(0)), new address[](0), address(s_mockRMN), address(s_sourceRouter));
    }
}
...
contract TokenPool_applyChainUpdates is TokenPoolSetup {
    ...
    function test_applyChainUpdates_Success() public {
        RateLimiter.Config memory outboundRateLimit1 =
            RateLimiter.Config({isEnabled: true, capacity: 100e28, rate: 1e18});
        RateLimiter.Config memory inboundRateLimit1 =
            RateLimiter.Config({isEnabled: true, capacity: 100e29, rate: 1e19});
        RateLimiter.Config memory outboundRateLimit2 =
            RateLimiter.Config({isEnabled: true, capacity: 100e26, rate: 1e16});
        RateLimiter.Config memory inboundRateLimit2 =
            RateLimiter.Config({isEnabled: true, capacity: 100e27, rate: 1e17});
    ...
    function test_applyChainUpdates_DisabledNonZeroRateLimit_Revert() public {
        RateLimiter.Config memory outboundRateLimit =
            RateLimiter.Config({isEnabled: true, capacity: 100e28, rate: 1e18});
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RateLimiter.DisabledNonZeroRateLimit.selector, chainUpdates[0].outboundRateLimiterConfig
            )
        );
        s_tokenPool.applyChainUpdates(chainUpdates);
    }
    ...
    function test_applyChainUpdates_NonExistentChain_Revert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TokenPool.NonExistentChain.selector, chainUpdates[0].remoteChainSelector)
        );
        s_tokenPool.applyChainUpdates(chainUpdates);
    }
...
contract TokenPool_setChainRateLimiterConfig is TokenPoolSetup {
    ...
    function test_Fuzz_SetChainRateLimiterConfig_Success(uint128 capacity, uint128 rate, uint32 newTime) public {
        ...
        RateLimiter.Config memory newOutboundConfig =
