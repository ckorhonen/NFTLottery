...
contract E2E is OnRampSetup, OffRampSetup {
        ...
        for (uint256 i = 0; i < s_sourceTokens.length; ++i) {
            ...
            address pool = address(
                new LockReleaseTokenPool(
                    IERC20(token), new address[](0), address(s_mockRMN), true, address(s_sourceRouter2)
                )
            );
    ...
    function test_E2E_3MessagesMMultiOffRampSuccess_gas() public {
        ...
        // Scoped to sending to reduce stack pressure
        {
            ...
            // Send messages
            messages1[0] =
                _sendRequest(1, SOURCE_CHAIN_SELECTOR, 1, s_metadataHash, s_sourceRouter, s_tokenAdminRegistry);
            messages1[1] =
                _sendRequest(2, SOURCE_CHAIN_SELECTOR, 2, s_metadataHash, s_sourceRouter, s_tokenAdminRegistry);
            messages2[0] =
            ...
            assertEq(
                balance0Pre - (messages1.length + messages2.length) * (i_tokenAmount0 + expectedFee),
        ...
        // Scoped to commit to reduce stack pressure
        {
            ...
            OffRamp.CommitReport memory report = OffRamp.CommitReport({
                priceUpdates: _getEmptyPriceUpdates(), merkleRoots: roots, rmnSignatures: rmnSignatures, rmnRawVs: 0
            });
        ...
        // Scoped to RMN and verify to reduce stack pressure
        {
            s_mockRMN.setTaggedRootBlessed(
                IRMN.TaggedRoot({commitStore: address(s_offRamp), root: merkleRoots[0]}), true
            );
            s_mockRMN.setTaggedRootBlessed(
                IRMN.TaggedRoot({commitStore: address(s_offRamp), root: merkleRoots[1]}), true
            );
