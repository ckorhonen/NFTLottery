...
contract CCIPHomeTest is Test {
    ...
    function _getConfigDigest(uint32 donId, Internal.OCRPluginType pluginType, bytes memory config, uint32 version)
        internal
        view
        returns (bytes32)
    {
            ...
            (PREFIX & PREFIX_MASK)
                | (uint256(
                        keccak256(
                            bytes.concat(
                                abi.encode(
                                    bytes32("EVM"), block.chainid, address(s_ccipHome), donId, pluginType, version
                                ),
                                config
                            )
                        )
...
contract CCIPHome_beforeCapabilityConfigSet is CCIPHomeTest {
    ...
    function test_beforeCapabilityConfigSet_success() public {
        ...
        callData = abi.encodeCall(
            CCIPHome.promoteCandidateAndRevokeActive,
            (DEFAULT_DON_ID, DEFAULT_PLUGIN_TYPE, newCandidateDigest, ZERO_DIGEST)
        );
    ...
    function test_beforeCapabilityConfigSet_InnerCallReverts_reverts() public {
        bytes memory callData =
            abi.encodeCall(CCIPHome.revokeCandidate, (DEFAULT_DON_ID, DEFAULT_PLUGIN_TYPE, ZERO_DIGEST));
...
contract CCIPHome_revokeCandidate is CCIPHomeTest {
    ...
    function test_revokeCandidate_ConfigDigestMismatch_reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(CCIPHome.ConfigDigestMismatch.selector, priorCandidateDigest, wrongDigest)
        );
...
contract CCIPHome_promoteCandidateAndRevokeActive is CCIPHomeTest {
    ...
    function promoteCandidateAndRevokeActive(Internal.OCRPluginType pluginType) public {
        ...
        s_ccipHome.promoteCandidateAndRevokeActive(
            DEFAULT_DON_ID, pluginType, secondConfigToPromote, firstConfigToPromote
        );
