...
contract NonceManager is INonceManager, AuthorizedCallers, ITypeAndVersion {
    ...
    function getIncrementedOutboundNonce(uint64 destChainSelector, address sender)
        external
    ...
    /// @inheritdoc INonceManager
    function incrementInboundNonce(uint64 sourceChainSelector, uint64 expectedNonce, bytes calldata sender)
        external
        onlyAuthorizedCallers
        returns (bool)
    {
