...
contract CCIPClientExample is CCIPReceiver, OwnerIsCreator {
    ...
    function sendDataPayNative(uint64 destChainSelector, bytes memory receiver, bytes memory data)
        external
        validChain(destChainSelector)
    {
        ...
        .ccipSend{value: IRouterClient(i_ccipRouter).getFee(destChainSelector, message)}(destChainSelector, message);
        emit MessageSent(messageId);
    }

    /// @notice sends data to receiver on dest chain. Assumes address(this) has sufficient feeToken.
    function sendDataPayFeeToken(uint64 destChainSelector, bytes memory receiver, bytes memory data)
        external
        validChain(destChainSelector)
    {
        Client.EVMTokenAmount[] memory tokenAmounts = new Client.EVMTokenAmount[](0);
    ...
    function sendTokens(uint64 destChainSelector, bytes memory receiver, Client.EVMTokenAmount[] memory tokenAmounts)
        external
        validChain(destChainSelector)
    {
        for (uint256 i = 0; i < tokenAmounts.length; ++i) {
