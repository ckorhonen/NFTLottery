...
abstract contract MultiOCR3Base is ITypeAndVersion, OwnerIsCreator {
    ...
    uint16 private constant TRANSMIT_MSGDATA_CONSTANT_LENGTH_COMPONENT_NO_SIGNATURES =
        4 // function selector
    ...
    uint16 private constant TRANSMIT_MSGDATA_EXTRA_CONSTANT_LENGTH_COMPONENT_FOR_SIGNATURES =
    ...
    function _setOCR3Config(OCRConfigArgs memory ocrConfigArgs) internal {
        ...
        if (ocrConfigArgs.isSignatureVerificationEnabled) {
            ...
            if (signers.length < transmitters.length) {
                revert InvalidConfig(InvalidConfigErrorType.TOO_MANY_TRANSMITTERS);
            }
    ...
    function _transmit(
        ...
        // Scoping this reduces stack pressure and gas usage
        {
            uint256 expectedDataLength =
                uint256(TRANSMIT_MSGDATA_CONSTANT_LENGTH_COMPONENT_NO_SIGNATURES) + report.length; // one byte pure entry in _report
