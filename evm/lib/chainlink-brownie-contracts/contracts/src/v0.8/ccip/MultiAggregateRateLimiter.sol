...
contract MultiAggregateRateLimiter is IMessageInterceptor, AuthorizedCallers, ITypeAndVersion {
    ...
    /// @inheritdoc IMessageInterceptor
    function onOutboundMessage(uint64 destChainSelector, Client.EVM2AnyMessage calldata message)
        external
    ...
    function _getTokenBucket(uint64 remoteChainSelector, bool isOutboundLane)
        internal
        view
        returns (RateLimiter.TokenBucket storage)
    {
    ...
    function currentRateLimiterState(uint64 remoteChainSelector, bool isOutboundLane)
        external
        view
    ...
    function updateRateLimitTokens(LocalRateLimitToken[] memory removes, RateLimitTokenArgs[] memory adds)
        external
        onlyOwner
    {
