...
contract CCIPHome is OwnerIsCreator, ITypeAndVersion, ICapabilityConfiguration, IERC165 {
    ...
    mapping(uint32 donId => mapping(Internal.OCRPluginType pluginType => VersionedConfig[MAX_CONCURRENT_CONFIGS]))
        private s_configs;
    ...
    function getConfigDigests(uint32 donId, Internal.OCRPluginType pluginType)
        public
        view
        returns (bytes32 activeConfigDigest, bytes32 candidateConfigDigest)
    {
        return (
    ...
    function getConfig(uint32 donId, Internal.OCRPluginType pluginType, bytes32 configDigest)
        external
        view
        returns (VersionedConfig memory versionedConfig, bool ok)
    {
    ...
    function getAllConfigs(uint32 donId, Internal.OCRPluginType pluginType)
        external
        view
        returns (VersionedConfig memory activeConfig, VersionedConfig memory candidateConfig)
    {
        ...
        VersionedConfig memory storedCandidateConfig =
            s_configs[donId][pluginType][_getCandidateIndex(donId, pluginType)];
    ...
    function promoteCandidateAndRevokeActive(
        ...
        if (s_configs[donId][pluginType][candidateConfigIndex].configDigest != digestToPromote) {
            revert ConfigDigestMismatch(
                s_configs[donId][pluginType][candidateConfigIndex].configDigest, digestToPromote
            );
        }
        ...
        return bytes32(
            (PREFIX & PREFIX_MASK)
                | (uint256(
                        keccak256(
                            bytes.concat(
                                abi.encode(bytes32("EVM"), block.chainid, address(this), donId, pluginType, version),
                                staticConfig
                            )
                        )
                    )
